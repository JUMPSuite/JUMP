...
sub generate_pub_tables {
	...
	my @arr=split /\//,$path;
        my $path= join "/",@arr[0..(scalar(@arr)-2)];
        #$path =~ s/(idsum_mod.db|sum_mod.db)$/publications\//;
	my $allPep=$path; my $uniPep=$path;
	$uniPep.='/publications/id_uni_pep.txt';
	$allPep.='/publications/id_all_pep.txt';
...
sub printProteinPublicationTables
{
	...
	my $input=$path;
#suresh fixed a small bug here by adding publication folder in front of file
	if ($unique) {$input.='/publications/id_uni_prot.txt';}
	else {$input.='/publications/id_all_prot.txt';}
