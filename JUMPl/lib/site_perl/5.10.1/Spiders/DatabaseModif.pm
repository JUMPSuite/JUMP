...
############ For each peptide in the peptide hash, change the mass value #######		
    foreach my $pepmass (keys(%$pephash))
    {
        foreach my $peptide (@{$$pephash{$pepmass}})
        {
			...
			my $flag        = 0;
		#	my @temp_peptide_array = split(/\./,$peptide);
		#	my $temp_peptide = $temp_peptide_array[1];
			...
			if ($flag != 0)
			{
				for (my $i = 1 ; $i <= $flag ; $i++)
				{
					...
					foreach my $c (@$iter)
					{
						...
						#$changed_peptide = $temp_peptide_array[0] . "." . $changed_peptide . "." . $temp_peptide_array[2];
				...
				############### mass = modif_mass * 1000		
							$modif_pepmass = $pepmass + $mid_hash{$join};
...
########replace the amino acid with modified amino acid ##############################
						...
						if (exists($mid_hash{$join}))
						{					
							next if ($changed_peptide=~/\.[a-zA-Z][^a-zA-Z]/);
							next if ($changed_peptide=~/[a-zA-Z][^a-zA-Z]\./);							
