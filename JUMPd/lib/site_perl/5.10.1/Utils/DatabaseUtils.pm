...
sub generateFastaAndPit {
	...
	print PIT "UniprotAC\tSJPGnumber\tGroupName\tAbundance\tResidueLength\tProteinName\tFullDescription\t$annotHeader\tDB_tier\n";
	...
	for (my $i = 0; $i < scalar(@priorities); $i++) {
		...
		## Setting of tier of proteins (according to the order of input .fasta files)
		## This will be used in jump -f when multiple tiers of filtering is required
		my ($protTier) = grep {(split(/\./, basename($inputFastas{$_})))[0] eq $priorities[$i]} keys %inputFastas;
		if (!defined $protTier) {
			$protTier = 0;
		} 
		...
		for (my $j = 0; $j < scalar(@pit); $j++) {
			for (my $k = 0; $k < scalar(@{$pit[$j]{'PROTEINS'}}); $k++) {
				...
				my $groupNumber = sprintf("SJPG%07d", ($j + 1 + $nGroups)).".".sprintf("%03d", ($k + 1));
				...
				my ($proteinName) = ($header =~ /^\>([a-z]{2}\|\S+\|\S+)/);
				my ($fullDescription) = ($header =~ /^\>[a-z]{2}\|\S+\|\S+\s(.+)$/);
				if ($priorities[$i] eq "contaminants") {
					$proteinName =~ s/Contaminant/Con/;
				}
				if (!defined $proteinName) {
					$proteinName = $protein;					
				}
				if (!defined $fullDescription) {
					$fullDescription = $protein;
				}
				print PIT "$protein\t$groupNumber\t$groupName\t$abundance\t$length\t$proteinName\t$fullDescription\t$proteinAnnot\t$protTier\n";
			}
		}
		$nGroups += scalar(@pit);
		...
		for (my $i = 0; $i < scalar(@priorities); $i++) {
			...
			for (my $j = 0; $j < scalar(@pit); $j++) {
				for (my $k = 0; $k < scalar(@{$pit[$j]{'PROTEINS'}}); $k++) {
					...
					print PIT "$protein\t$groupNumber\t$groupName\t$abundance\t$length\t$proteinName\t$fullDescription\t$decoyAnnot\t$protTier\n";
				}
			}
			$nGroups += scalar(@pit);
...
sub digest {
	...
	## No cleavage rule; K and R before P (e.g. KP, RP) are not cleaved
	my $seq = shift (@_);
	$seq =~ s/([KR])(?!P)/$1 /g;
	$seq =~ s/ \Z//;
	my @allPeptides = split(/ /, $seq);
	my @selPeptides;
...
sub generateDbName {
	...
	if (int($$params{'add_Nterm_peptide'}) == 229 && int($$params{'add_K_Lysine'}) == 229) {
		...
		print "  Note that \"add_Nterm_peptide\" and \"add_K_Lysine\" are different\n";
		print $log "  Note that \"add_Nterm_peptide\" and \"add_K_Lysine\" are different\n";
	}
