...
#    my $prec_mz_charge = <DTAFILE>;
    my ($prec_mass,$prec_charge) = ($dta->get_prec_mz(),$dta->get_charge());#split(/\s+/,$prec_mz_charge);

    %mz_hash = %{$dta->get_mz_int_hash()};
#     while(<DTAFILE>)
#     {
...
#	foreach my $mz (@mz_array)
	...
	if(abs($mz-216.042020900552)<(216.042020900552*$parameter->{'ppm'})/1000000)
	{
	    ...
	    open(OUT,">" . $dta->get_dta_file() . ".y-immo");
...
#    open(OUT,">" . $dta->get_dta_file() . ".barger");
#    print OUT $dta->get_prec_mz() . " " . $dta->get_charge() . "\n";#$prec_mz_charge;
    my (@new_mz, @new_int);
