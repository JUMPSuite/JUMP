...
#########################################################################	
    my %dtahash= %{$dta->get_mz_int_hash()};#%{$dtahash_orig->{'ms2'}};


    my $param = $self->get_parameter();
    my $TMT_data_start_mz = 0;
    my $startmz = (sort {$a <=> $b} keys %dtahash)[0];
    my $prec_mz = $dta->get_prec_mz();#$dtahash_orig->{'prec_mz'};
    my $charge = $dta->get_charge();#$dtahash_orig->{'prec_charge'};
...
# =cut	
#    system(qq(rm $dta));
#    open (DTA, ">" . $dta->get_dta_file() . ".chez");
...
#			}			
 ...
 #   close(DTA);
    $dta->update(\@new_mz,\@new_int);
#    $dta->write_dta_file_arrorder();
}
