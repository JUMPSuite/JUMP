...
sub get_msms_dta
{
    ...
    my $dta = $self->get_dta();
    my $scan;
    if($dta->get_dta_file() =~ /(([A-Za-z0-9\_\-]+)\.(\d+)\.(\d+)\.(\d+).dta)\Z/)
    {
	$scan = $3;
    }
 #   open(DTA,$dta->get_dta_file()) || die "can not open the dta file: $dta\n";
 #   my $header = <DTA>;
 #   my ($mz,$charge) = split(/\s+/,$header);
    my $mz = $dta->get_prec_mz();
    ...
    my @mz_array=@{$dta->get_mz_array()};#();
    my @int_array=@{$dta->get_int_array};#();
    # while(<DTA>)
    # {
    # 	my ($mz,$int) = split(/\s+/,$_);
    # 	push(@mz_array,$mz);
