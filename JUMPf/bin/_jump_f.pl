...
my (%sumhash, %modsumhash, $database, %dbhash,$pepXML_mode,%usedDBs);
	...
	# added on 11/5/19: for comet search to deal w/ multi-fractions
	my $cometFlag=0;
	$cometFlag=1 if ($paramhash{search_engine} eq 'comet');
	# end added on 11/5/19
...
###### end of addition
	...
	system("cp $paramsfile $group_dir/jump_f.params");
...
close (DTAFILES);
	...
	# added on 1/31/19: for comet search to deal w/ multi-fractions
	#my $cometFlag=0;
	#$cometFlag=1 if ($paramhash{search_engine} eq 'comet');
	# end added on 1/31/19

	for my $run (@$members){ # loop for each run

		# re-set search_engine for comet
		$paramhash{search_engine}='comet' if ($cometFlag);

		# initialization
		my %del; $del{'contaminants'} = 0; $del{'length'} = 0; $del{'mixlabel'} = 0; $del{'maxmis'}=0; $del{'maxmod'}=0; $del{'DX'}=0; $del{'sharedecoy'}=0; $del{'no_required_mod'}=0; $del{'ppi_filtered'}=0; $del{DB_composite_filtered}=0;
		...
		# parse jump.params
		if (defined($paramhash{search_engine}) and $paramhash{search_engine} eq 'comet') {
			$idutils->parse_COMETparams(\%paramhash, $run);
		} else {
		...
		# DB_composite_filtered
		my (%pro2DBtier);
		my $DB_tier=build_pro2DBtier($paramhash{pit_file}, \%pro2DBtier);

		# out file mode
		if ($paramhash{search_engine} eq 'sequest' or $paramhash{search_engine} eq 'comet') {
		...
		}elsif ($paramhash{search_engine} eq 'comet') { 
			#$xml =~ s/pepcXML$/pepXML/;
			$xml =~ s/pepcXML$/pep.xml/;
			if (-e $xml) { $pepXML_mode=1; } 
			else { 
				$xml =~ s/pep.xml$/pepXML/;
				if (-e $xml) { $pepXML_mode=1; } else {	$pepXML_mode=0; }
			}
		}
#$pepXML_mode=0;
		...
		# added on 1/24/19
		# from now on, we treat comet the same way as sequest
		$paramhash{search_engine} = 'sequest' if ($paramhash{search_engine} eq 'comet');
		...
		for my $outfile (@out){
			...
			my ($db) = $idutils->pepXMLhashesTransfer(\%paramhash, $run, $outfile, \%paraHash, \%outInforHash, \%peptidehash, \%runhash, \%del, \$blank,  $DB_tier, \%pro2DBtier);
...
#		while (my ($outfile, $hash) = each %{$runhash{$run}}){ if ($$hash{'protein'} =~ /^(Random__|Decoy__)/){ $bad++; } else { $good++; }	}
	...
	if (defined($paramhash{DB_composite})) {
		print "Removing $del{DB_composite_filtered} outfiles due to composite database filtering\n";
		print LOGFILE "Removing $del{DB_composite_filtered} outfiles due to composite database filtering\n";
	}
...
#=head
		...
		rm_multiple_charge_for_single_precursor($runhash{$run},\%peptidehash,\%del,$paramhash{search_engine});
...
#			if ($$hash{'protein'} =~ /^Random__/ || $$hash{'protein'} =~ /^Decoy__/){ $bad++;
	...
	# Bypass Filtering 
	if (defined($paramhash{database}) and $paramhash{database} ne '0') { $database=$paramhash{database}; }
...
#This is added by suresh: Die is removed but print is added in log file
        foreach my $pro (keys %proteinhash) { my $seq=$proteinhash{$pro}{sequence}; if (length($seq)==0) { print LOGFILE "The bad protein $pro sequence: $seq\n\n"; } }
		if ($paramhash{'combine_filtermethods'}){
			print "\n";
		...
		for my $protein (keys %tempproteinhash){
			...
			for my $peptide (keys %{$tempproteinhash{$protein}{'peptides'}}){
				...
				if ($dCn <= $paramhash{'one_hit_wonders_min_dCn'}){
					...
					#if (scalar(keys %{$peptidehash{$peptide}{'proteins'}})==0) { delete $peptidehash{$peptide};} # added on 11/16/16 when Ji-Hoon found the bug
					next;
				}
				if ($charge == 1){
					if ($XCorr <= $paramhash{'one_hit_wonders_min_XCorr_z1'}){
						delete  $tempproteinhash{$protein};
						   ...
						   $onehitpeptides{$peptide} = $peptidehash{$peptide}{'orig_peptide'};
    	   					   ...
    	   					   delete $peptidehash{$peptide}{'proteins'}{$protein};
					if (scalar(keys %{$peptidehash{$peptide}{'proteins'}})==0) { delete $peptidehash{$peptide};}
						next;
					} 
				} elsif ($charge == 2){
...
##############################################
	...
	# save peptidehash
	store \%peptidehash,"$group_dir/misc/peptidehash.hash";
...
#			$html->gen_IDHtml(\%modproteinhash, \%modpeptidehash, \%fprhash, $modgroup_dir, $database, $modgroup_num, $modsubgroup_num, 0, 1);
			...
			# print decoys
			if($paramhash{'keep_decoy'} eq '1') {
				my $txtfile="IDwDecoy_mod.txt";
				save_Filtering($modgroup_dir,\%modproteinhash,\%modpeptidehash,$txtfile);
			}

			# save peptidehash
			if (!(-e "$modgroup_dir/misc/")) {
				system(qq(mkdir -p $modgroup_dir/misc/));
			}
			store \%modpeptidehash,"$modgroup_dir/misc/peptidehash.hash";
...
=cut
	...
	if ( $paramhash{unique_protein_or_peptide} eq 'protein' )
	{
#suresh added this section for proteogenomics table
                #my $geneMatrixInput = $dir."/publications/id_uni_prot.txt";
                #print "Converting $geneMatrixInput file into Unique gene column mapped to best protein out file.\n\n";
                #my @cmd = ("python $Bin/protmat2genemat.py $geneMatrixInput");
                #system(@cmd);
#end suresh code
...
{
	my ($runhash,$peptidehash,$del,$searchEngine)=@_;
        ...
        #while (my ($run,$runhash) = each %scanhash){# %runhash{$scan}{$outfile}
                #for my $hash (values %$runhash){# %hash{$outfile}
					...
					# }
							...
							die "Error: searchEngine specified in rm_multiple_charge_for_single_precursor(): $searchEngine\n";
						}
                                        }
	...
	while(my ($pep, $hash) = each %{$peptidehash})
	{
		...
		if ($best_protein ne ''){
		    ...
		    $$best_proteins{$best_protein}='';
                    ...
                    print LOGFILE "The peptide that mapped to empty best protein is $pep\n";
                    }
	}
	#print scalar(keys %{$best_proteins}),"\n";
...
sub build_pro2DBtier
{
	my ($pitfile,$pro2DBtier)=@_;
	# pit initialization:
	my $use_pit=1;
	my ($grouphash,$decoy_grouphash);
	#open(PIT,"HUMAN.pit");
	...
	while(<PIT>)
	{
		...
		my $db_tier=''; # database tier for this protein
		if ($DB_tier) {
			my @tmp=split /\t/, $annotationCols;
			$db_tier=pop @tmp;
			$annotationCols=join("\t",@tmp);

			$$pro2DBtier{$protein}=$db_tier;
		}
	}
	return 1;
}
