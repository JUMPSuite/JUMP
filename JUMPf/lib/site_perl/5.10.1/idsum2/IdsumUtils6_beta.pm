...
sub parse_COMETparams
{
	...
	if (1)
	{
        ...
        my $paramfile = "$fraction\/comet.params";
	unless (-e $paramfile) { $paramfile = 'comet.params'; }

        while (!(-e $paramfile)){
                print "\n$paramfile does not exists!!!";
                print LOGFILE "\n$paramfile does not exists!!!";
                $paramfile = idsum2::CL->ask("\nPlease enter the correct full path to the comet.params parameter file.", "");
        }
        ...
        while (<IN>)
        {
                ...
                if ($key =~ /^variable_mod/)       #dynamic mod
                {
			my ($massDiff,$residues)=split /\s+/,$value;
			my @aa=split //,$residues;
			foreach my $a (@aa) { $$seqparamhash{$fraction}{"dynamicmods"}{$a} = $massDiff;}
                        next;
                }
...
sub parse_badproteinfile{
	...
	while (<IN>){
		...
		$$proteinhash{$protein}++;
#print "------This is bad protein    $protein\n";
#print "------This is bad protein value   $$proteinhash{$protein}\n";
		if ($protein =~ /\|/){
			my @proparts = split('\|', $protein);
			my $shortpro = $proparts[scalar(@proparts)-1];
#print "------This is complicated    $shortpro\n";
...
sub parse_jumpparams
{
	...
	if (defined($$seqparamhash{jump_params}) and $$seqparamhash{jump_params} ne '0') {
		$paramfile = $$seqparamhash{jump_params};
	} else {
		$paramfile = "$fraction\/jump.params"; #print "$paramfile\n";
...
print "The peptide protein pairs ---- $intpep\t$protein\n";
	if ($noRNAseq) {} else {
	if ( $protein =~ /Decoy/ ) 
	{ 
...
sub create_proteinhash
{
	...
	{
#print "----the peptide hash peptide is $peptide\n";		
		$peptidenum++;
#		printf "\rCreating proteinhash with Peptide_Hash: %d%% done", ($peptidenum/$pepnum)*100;
		...
		{       
#print "----the peptide hash protein is $protein\n";
			$protein=~s/\#\#//;
                        #print "\n$protein\n";
...
#print "-----The accesion is   $protein\n";
                                #print "-----The sequence is   $$proteinhash{$protein}{'sequence'}\n";
				if ( length($$proteinhash{$protein}{'sequence'})==0 ) 
                                { 

                        #die "Zero length protein:$protein!!!\n";
                        }
...
{
  shift @_;
  my ($paramhash,$run,$outfile,$paraHash,$outInforHash,$peptidehash,$runhash,$delhash,$blank,$DB_tier,$pro2DBtier,$cpr,$cpn,$pep2reads) = @_;
  ...
  if ($$paramhash{search_engine} eq 'jump') {
	$outfile =~ m/\.(\d+)\.(\d+)\.(\d)\.spout/;
  	($scan,$charge) = ("$1.$2",$3);
	$outkey =~ s/(\.spout)//;  #print "$outkey\n";
  #} elsif ($$paramhash{search_engine} eq 'sequest' or $$paramhash{search_engine} eq 'comet') {
  } elsif ($$paramhash{search_engine} eq 'sequest') {
	$outfile =~ m/\.(\d+)\.(\d+)\.(\d)\.out/;
  	($scan,$charge) = ("$1.$2",$3);
	$outkey =~ s/(\.out)//;  #print "$outkey\n";
  } else {
	die "Unrecognized search_engine: $$paramhash{search_engine}\n";
  }
  ...
  my $minDB_tier=100; # to record the minimum level of DB tier (the smaller, the higher priority)
  my $hit_bestDBtier=100; # to record the best DB tier of a specific hit (e.g., out of the 10 matches listed in the outfile)

  for (my $i=0; $i<$$outInforHash{$outkey}{hitShown}; $i++)
  {
    # pass values from %outInforHash
    my $k=$i+1;# corespond to %outInforHash
    my ($a, $rank, $id, $expmass, $c, $XCorr, $Sp, $e, $protein);

    if ($$paramhash{search_engine} eq 'jump') {
	($a, $rank, $id, $expmass, $c, $XCorr, $Sp, $e, $protein) = ($k,'','',$$outInforHash{$outkey}{'calc_neutral_pep_mass'}[$k]+$Hydrogen_mass,0,$$outInforHash{$outkey}{'WeightedEvalue'}[$k],'',"$$outInforHash{$outkey}{'num_matched_ions'}[$k]\/$$outInforHash{$outkey}{'tot_num_ions'}[$k]",$$outInforHash{$outkey}{'protein'}[$k]);
    } else {
	($a, $rank, $id, $expmass, $c, $XCorr, $Sp, $e, $protein) = ($k,'','',$$outInforHash{$outkey}{'calc_neutral_pep_mass'}[$k]+$Hydrogen_mass,0,$$outInforHash{$outkey}{'xcorr'}[$k],'',"$$outInforHash{$outkey}{'num_matched_ions'}[$k]\/$$outInforHash{$outkey}{'tot_num_ions'}[$k]",$$outInforHash{$outkey}{'protein'}[$k]);
    }
    ...
    # 4/26/19
    # to compromise a potential bug by JUMP, when the sequence entry is short
    # to make sure the flanking AA is either [A-Z] or '-'; otherwise, turn it to '-'
    unless ($$outInforHash{$outkey}{'peptide_prev_aa'}[$k] =~ m/[A-Z\-]/) {
	$$outInforHash{$outkey}{'peptide_prev_aa'}[$k] = '-';
    }
    unless ($$outInforHash{$outkey}{'peptide_next_aa'}[$k] =~ m/[A-Z\-]/) {
	$$outInforHash{$outkey}{'peptide_next_aa'}[$k] = '-';
    }
    # end of edit on 4/26/19
    ...
    # ATTENTION!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    # All the remaining in this for loop: only for 1st protein name (for most casese), unless 1) dCn<0.5; and 2) peptide similarity < 0.9
    my $Decoy=0;
    if ($noRNAseq) 
    { 
      if ($i==0) 
	...
	if ($DB_tier and defined($$pro2DBtier{$protein}) and $$pro2DBtier{$protein}<$minDB_tier){
		$minDB_tier=$$pro2DBtier{$protein};
	}
      } 
    }
    else {_update_max_levels(\$mxlv,\$mxtd,\$mxG,\$mxRpk,\$mxProName,\$mxRank,$protein,$cpr,$cpn,$i);}
...
#  print "$outfile,$i,$k,$j,$tmpP,$tmpProteins[$i][$j]\n";
	...
	if ($DB_tier and defined($$pro2DBtier{$tmpP}) and $$pro2DBtier{$tmpP}<$minDB_tier){
		$minDB_tier=$$pro2DBtier{$tmpP};
	}
    ...
    if ($DB_tier and $hit_bestDBtier>$minDB_tier) { # if the 2nd hit has better DB tier, replace the 1st hit w/ the 2nd
	$hit_bestDBtier=$minDB_tier;
	$mxRank=$i;
	$mxProName=$protein;
    }
  ...
  #if ($noRNAseq) { $mxlv=1; $mxRank=0;} # updated on 9/26/17
  if ($noRNAseq) { $mxlv=1;}
...
=cut
  ...
  #my ($a, $rank, $id, $expmass, $c, $XCorr, $Sp, $e, $protein, $g, $h) = split(/\s/, $temp);
  my ($a, $rank, $id, $expmass, $c, $XCorr, $Sp, $e);
  my $precursor_peak_intensity_percentage='n/a'; 
  if ($$paramhash{search_engine} eq 'jump') {
  	if($database=~/mdx/) {$database=~s/\.mdx$//;}
	($a, $rank, $id, $expmass, $c, $XCorr, $Sp, $e) = ($k,'','',$$outInforHash{$outkey}{'calc_neutral_pep_mass'}[$k]+$Hydrogen_mass,0,$$outInforHash{$outkey}{'WeightedEvalue'}[$k],'',"$$outInforHash{$outkey}{'num_matched_ions'}[$k]\/$$outInforHash{$outkey}{'tot_num_ions'}[$k]");
	$precursor_peak_intensity_percentage=$$outInforHash{$outkey}{'precursor_peak_intensity_percentage'}; 
  } else {
  	if($database=~/hdr/) {$database=~s/\.hdr$//;}
	($a, $rank, $id, $expmass, $c, $XCorr, $Sp, $e) = ($k,'','',$$outInforHash{$outkey}{'calc_neutral_pep_mass'}[$k]+$Hydrogen_mass,0,$$outInforHash{$outkey}{'xcorr'}[$k],'',"$$outInforHash{$outkey}{'num_matched_ions'}[$k]\/$$outInforHash{$outkey}{'tot_num_ions'}[$k]");
  }

  my $protein=$mxProName;

  if (defined($DB_tier) and 
	defined($$paramhash{DB_composite}) and $$paramhash{DB_composite} > 0 and
	defined($$pro2DBtier{$protein}) and $minDB_tier > $$paramhash{DB_composite}){
	$$delhash{DB_composite_filtered}++;
	return;
  }
