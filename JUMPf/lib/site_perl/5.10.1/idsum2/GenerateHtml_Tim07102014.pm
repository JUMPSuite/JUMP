...
sub gen_sumIDHtml {
	...
	my $total_SC_index = $index_i;
	$CGILINK .= "\$('td:eq($total_SC_index)', nRow).html('<a href=\"$http/id_shared_unique.cgi?protein=' + aData[1] + '&dir=$save_dir\">' + aData[$total_SC_index] + '</a>')\n";
	...
	my $total_peptide_index = $index_i + 1;
	$CGILINK .= "\$('td:eq($total_peptide_index)', nRow).html('<a href=\"$http/id_total.cgi?protein=' + aData[1] + '&dir=$save_dir&database=$database\">' + aData[$total_peptide_index] + '</a>');\n";
						...
						} keys %$grouphash)
	{
		...
		if($grouphash->{$protein}->{'order'}==1)
		{
			...
			# Contemplating to change this to include modification? 20150615
			print TXT "$$Protein_Hash{$protein}{'total_nomod'};";
...
sub gen_IDHtml {
						...
						} keys %$grouphash)
	{
		...
		#added if loop DMD 5/19/05
			...
			# this is the total peptide
			#$HTMLTABLE .= "<td align=\"center\">" . $$Protein_Hash{$protein}{'total_nomod'} . "</td>\n";
			my $total_protein_hash_peptides = keys %{$$Protein_Hash{$protein}{'peptides'}};
			$HTMLTABLE .= "<td align=\"center\">" . $total_protein_hash_peptides . "</td>\n";
...
sub gen_IDwGHtml{
	...
	$redirect_path = "http://$server$remove/html/IDwG.html";
...
#    printf HTML "<TD Align=center><Font Size=2>Protein FDR = %.2f%%,&nbsp  Peptide FDR = %.2f%% ", $$fprhash{'protein_fpr'}, $$fprhash{'peptide_fpr'};
	...
	## changed the following on 2015/06/04
	#$index_i = $index_i + 1; 
	#$HTMLTABLEHEADER .= "<th style=\"border-bottom:3px solid black\">Abundance</th>\n";
...
$html_page .= "\"aLengthMenu\": [\n";
     ...
     $html_page .= "\"fnRowCallback\": function( nRow, aData, iDisplayIndex ) {\n";
			...
			#
	if (!($TITLE eq "Multiple ID Summary with Groups for Modifications") && !($TITLE eq "Multiple ID Summary without Groups for Modifications")) {
            $html_page .= $CGILINK;
    	}
