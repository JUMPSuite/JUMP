...
sub get_protein_name
{
	shift @_;
	my ($hash,$out)=@_;
	
	if (defined($$hash{$out}{protein}[1])) { return $$hash{$out}{protein}[1]; }
	else { return ''; }
}
...
sub printPSMinfor
{
	...
	foreach my $out (keys %$hash)
	{
		...
		elsif (defined($$hash{$out}{expect}[1])) { $score=$$hash{$out}{expect}[1]; $deltaScore=$$hash{$out}{deltacn}[1]; }
		...
		my $peptide=$$hash{$out}{fullPeptide}[1];
		my $peptide_prev_aa=$$hash{$out}{'peptide_prev_aa'}[1];
		my $peptide_next_aa=$$hash{$out}{'peptide_next_aa'}[1];
		my $fullpep=join(".",$peptide_prev_aa,$peptide,$peptide_next_aa);
		my $protein=$$hash{$out}{protein}[1];
		my $TD; if ($protein =~ m/$decoyPattern/) {$TD='decoy'; $protein =~ s/\#\#//;} else {$TD='target';}

		print OUT "$out\t$score\t$deltaScore\t$precursor_neutral_mass\t$calc_neutral_pep_mass\t$fullpep\t$protein\t$TD\n";
	}
...
sub initialize_outfile
{
	...
	$$newhash{$out}{'precursor_peak_intensity_percentage'}=$$hash1{'precursor_peak_intensity_percentage'};
...
sub add_filtered_outfile
{
	...
	for ( my $i=1; $i<=$$hash1{'hitShown'}; $i++ )
	{
		...
		#my $fullpep=$$hash1{'fullPeptide'}[$i];
		my $pro=$$hash1{'protein'}[$i];
		my $peptide_prev_aa=$$hash1{'peptide_prev_aa'}[$i];
		my $peptide_next_aa=$$hash1{'peptide_next_aa'}[$i];
		my $fullpep=join("",$peptide_prev_aa,$pep,$peptide_next_aa);
		#print "$out  $fullpep  $pep\n";

		#if ( $pro =~ /Decoy/ ) { $pep=orig_pep_for_decoy($pep,'R2'); }
		if ( $pro =~ /Decoy/ ) { $pep=orig_pep_for_decoy($fullpep,'R'); }
		#if ( $pro =~ /Decoy/ ) { $pep=orig_pep_for_decoy($fullpep,'R'); print "$out  $fullpep  $pep\n";}
...
=cut
	#elsif ( $decoy_strategy eq 'R' )
	if ( $decoy_strategy eq 'R' )
        {
		#R.HDWSGPLSLCSFR.B
                $peptide =~ s/[\@\#\*\^\~\$\.]//g;
		chop($peptide);
...
sub add_outfile_specificHit
{
# goal: same as add_outfile(), except only adding PSM of a particular hit (rather than all 10 hits within the original outfile)
	shift @_;
	my ($paraHash,$oldhash,$newhash,$out,$hit)=@_;
	...
	if (!defined($hit) || $hit>$$oldhash{$out}{'hitShown'}) {
		die "Error in add_outfile_specificHit(): $out, hit not defined or >hitShown: hit=$hit, hitShown=$$oldhash{$out}{'hitShown'}\n";
	}

	add_psm($paraHash,$hash1,$hash3,$hit);
...
sub merge_outfiles
{
	...
	$$outhash3{$out}{'precursor_peak_intensity_percentage'}=$$hash1{'precursor_peak_intensity_percentage'};
...
sub printPepXML
{
        my ($self,$seqPara, $outInfor, $outXML, $topHit, $noDecoy)=@_;
	my $run='';
        ...
        for my $outfile (@out)
        {
		next if (defined($noDecoy) and $noDecoy and $$outInfor{$outfile}{'protein'}[1] =~ /Decoy/); # check whether to skip printing decoys
...
sub pepXML2Hashes
{
	...
	while (<IN>)
	{
		...
		if (/search_engine=/ and /precursor_mass_type=/ and /fragment_mass_type=/)
		{
			chomp;
			s/^<//; s/>$//;
			my @t=split /\s+/,$_;
			...
			if (!defined($$paraHash{'out_data_type'})) { $$paraHash{'out_data_type'}='out';}
			if (!defined($$paraHash{'out_data'})) { $$paraHash{'out_data'}='.out';}
			next;
		}
		
		#modifications
		#Suresh added this line
...
#print "@tmp\n";
			...
			# scan
			#$outfile =~ m/\.(\d+)\.(\d+)\.(\d)$/;
			$$outInforHash{$outfile}{'scan'}=$$outInforHash{$outfile}{start_scan};
			$$outInforHash{$outfile}{'charge'}=$$outInforHash{$outfile}{assumed_charge};
...
=head
		...
		if (/\<spectrum_query spectrum=\"(.*?)\" start_scan=\"(\d+)\" end_scan=\"\d+\" precursor_neutral_mass=\"([\d\.]+)\" precursor_peak_intensity_percentage=\"([\d\.\%]+)\" precursor_peak_intensity_order=\"\d+\" assumed_charge=\"(\d+)\" index=\"\d+\"\>/)
		{
			...
			$$outInforHash{$outfile}{'precursor_peak_intensity_percentage'}=$4;
			$$outInforHash{$outfile}{'charge'}=$5;
