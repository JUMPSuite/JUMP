...
sub pepXML2Hashes
{
	...
	while (<IN>)
	{
		...
		#optional parameters
		if (/\<parameter name\=\"(.*?)\" value\=\"(.*?)\"\/\>/) {$$paraHash{$1}=$2; next;}
	...
	while (<IN>)
	{
		...
		if (/^\<spectrum_query spectrum=\"(.*?)\" start_scan=\"(\d+)\" end_scan=\"\d+\" precursor_neutral_mass=\"([\d\.]+)\" precursor_peak_intensity_percentage=\"([\d\.\%]+)\" precursor_peak_intensity_order=\"\d+\" assumed_charge=\"(\d+)\" index=\"\d+\"\>/)
		{
			...
			#print "$outfile  $$outInforHash{$outfile}{'precursor_peak_intensity_percentage'}  jump\n";
			
			$$outInforHash{$outfile}{'hitShown'}=0;
			next;
		}#else {die "unmatched pattern:\n$_";}
		# for sequest
		...
		#optional parameters
		if (/\<search_score name=\"(\w+)\" value=\"(.*?)\"\/\>/) {$$outInforHash{$outfile}{$1}[$$outInforHash{$outfile}{'hitShown'}]=$2; next;}
