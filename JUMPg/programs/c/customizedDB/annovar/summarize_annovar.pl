...
our $VERSION = 			'$Revision: 514 $';
our $LAST_CHANGED_DATE =	'$LastChangedDate: 2013-02-11 09:31:25 -0800 (Mon, 11 Feb 2013) $';
...
GetOptions('verbose|v'=>\$verbose, 'help|h'=>\$help, 'man|m'=>\$man, 'outfile=s'=>\$outfile, 'buildver=s'=>\$buildver, 'step=s'=>\$step, 
	'checkfile!'=>\$checkfile, 'remove'=>\$remove, 'verdbsnp=s'=>\$verdbsnp, 'ver1000g=s'=>\$ver1000g, 'genetype=s'=>\$genetype,
	'veresp=s'=>\$veresp, 'alltranscript!'=>\$alltranscript) or pod2usage ();
...
if ($valistep{7}) {
	while (<STEP7>) {
		m/^snp\d+\w+?\t(\S+)\t(\S+\s+\S+\s+\S+\s+\S+\s+\S+)/ or die "Error: invalid record found in annovar outputfile 7 : <$_>\n";
		$allstep[7]->{$2} = $1;
	}
}
...
if ($valistep{14}) {
	while (<STEP14>) {
		m/^esp\d+\w*_all\t(\S+)\t(\S+\s+\S+\s+\S+\s+\S+\s+\S+)/ or die "Error: invalid record found in annovar outputfile 10 : <$_>\n";
		$allstep[14]->{$2} = $1;
	}
}
...
=head1 SYNOPSIS
 ...
 Example: summarize_annovar.pl -ver1000g 1000g2012apr -verdbsnp 137 -veresp 6500si -alltranscript -remove -buildver hg18 ex2.human humandb/
 
 Version: $LastChangedDate: 2013-02-11 09:31:25 -0800 (Mon, 11 Feb 2013) $
...
version of 1000 Genomes Project dataset to use in annotation. By default, 
1000g2010nov is used.

=item B<--veresp>

version of ESP dataset to use in annotation. By default, 5400 is used, but users 
can also choose 6500 or 6500si.
...
specify that the program should print out all transcripts for an exonic mutation. By 
default, only one random transcript will be included in the output.
