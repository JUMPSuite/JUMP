...
while(<IN>)
{
	...
	for my $i (1..($#a-1))
	{
		if ( length($a[$i])>=$minFragmentLength )
		{
			print OUT '>',$idInfor->{readID},'|',$idInfor->{strand},$idInfor->{offset},'|',$ps{$i}{start},'-',$ps{$i}{end},"\n";
			my @b=split(//,$a[$i-1]); my @c=split(//,$a[$i+1]);
			print OUT "$b[$#b]$a[$i]$c[0]\n";
		}
	}
=head
	#for last fragment
	if (length($a[$#a])>=$minFragmentLength)
	{
