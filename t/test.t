...
# relative everything
system("perl jump_sj.pl -p $param_file $mz_file &> /dev/null");
ok( !$?, "relative params, relative mzXML" );
move( File::Spec->join( $mz_root, basename($mz_file) ), $mz_file );
File::Path->remove_tree( $mz_root );

# absolute MZ file
system("perl jump_sj.pl -p $param_file $abs_mz_file &> /dev/null");
ok( !$?, "relative params, absolute mzXML" );
move( File::Spec->join( $mz_root, basename($mz_file) ), $mz_file );
File::Path->remove_tree( $mz_root );

# absolute param file
system("perl jump_sj.pl -p $abs_param_file $mz_file &> /dev/null");
ok( !$?, "absolute params, relative mzXML" );
move( File::Spec->join( $mz_root, basename($mz_file) ), $mz_file );
File::Path->remove_tree( $mz_root );

# absolute everything
system("perl jump_sj.pl -p $abs_param_file $abs_mz_file &> /dev/null");
