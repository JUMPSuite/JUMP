...
if (!defined($parahash{pretest_function}) or $parahash{pretest_function}==0)
{
	...
	# added on 5/2/17
	# print data matrix
	printData(\%data, \@tmtarray, "$outputDir\/wholeData.txt",\%featurehash);
	...
	foreach my $pct (sort{$b<=>$a} keys %{$parahash{cluster_top_pct}})
	{
		...
		if (scalar(keys %data)>1)	# check if hash is empty (in case dataset is too small)
		{
			...
			printHeatmap(\%data, $pct, \@tmtarray, $outputDir, $parahash{'bypass_row_clustering'},$parahash{color_low},$parahash{color_mid},$parahash{color_high});
		}
	}
	# rm 'Rplots.pdf'
...
sub printLog2RatioDistributions
{
	...
	pdf(file=paste(outputDir,'/log2ratio_distributions.pdf',sep=''),width=ncol(m),height=ncol(m))
...
sub printData
{
	my ($data,$header,$output,$featurehash)=@_;

	my $rsd=(defined($featurehash))?1:0; # print rsd and rank?
	...
	foreach my $p (keys %{$data})
	{
		...
		if ($rsd) {
			print OUT "\t$$featurehash{$p}{rsd}\t$$featurehash{$p}{rankpct}";
		}
...
sub printHeatmap
{
	my ($data,$pctCut,$header,$outputDir,$bypass_row_clustering,$color_low,$color_mid,$color_high)=@_;
	...
	$R -> set('color_low', $color_low);
	$R -> set('color_mid', $color_mid);
	$R -> set('color_high', $color_high);
	$R -> run(q`data=read.table(paste(outputDir,"/.tmpdata",sep=''),head=T,sep="\t")
	m=as.matrix(data[,2:ncol(data)])
	rownames(m)=data[,1]
	library(gplots)
	#mycol <- colorpanel(n=20,low="blue",mid="white",high="red")
	mycol <- colorpanel(n=20,low=color_low,mid=color_mid,high=color_high)
	par(mar=c(12,4,6,12))
	pdf(file=paste(outputDir,'/heatmap_',pctCut,'pct.pdf',sep=''),width=10,height=10)
	#pdf(file=paste(outputDir,'/heatmap_',pctCut,'pct.pdf',sep=''),width=ncol(m),height=ncol(m))
...
sub parse_params
{
		...
		s/\s*([;\#].*)$//; # delete comments
                ...
                #if ($key =~ /^sig1[23]/)
                if ($key =~ /^sig1[23]/ or $key =~ /^batch(.*?)_sig1[23]/)
                {
...
sub parse_input
{
	...
	while (<IN>)
	{
		...
		if (/sig126/ or /sig127N/ or /sig127C/ or /sig128N/ or /sig128C/ or /sig129N/ or /sig129C/ or /sig130N/ or /sig130C/ or /sig131/ ) # header line
		{
			$headerline=0;
