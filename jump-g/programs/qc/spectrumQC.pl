#!/usr/bin/perl -I /home/yli4/development/JUMPg/HPC_version/090118_fullversion/programs/f
use strict;
use warnings;

use FindBin qw($Bin);
my $lda_R_script="$Bin/qc_LDA.R";
...
rLDA_analysis($outputDir,$parahash{'PSM_recoveray_rate'},$lda_R_script);
...
sub rLDA_analysis
{
	my ($outputDir,$PSM_recoveray_rate,$R_script)=@_;
...
sub printPSMinfor
{
	...
	foreach my $out (keys %$hash)
	{
		...
		my $precursor_matched_peptides='n/a';
		if (defined($$hash{$out}{precursor_matched_peptides})) {
			$precursor_matched_peptides=$$hash{$out}{precursor_matched_peptides};
		}
