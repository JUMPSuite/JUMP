...
if (1)
{
	system(qq(cd $output_dir && $parahash{'programs'}{params}));
...
if (1)
{
	...
	if (defined($parahash{$mode}) and $parahash{$mode} ne '0')
	{
		...
		else
		{
			...
			system(qq(cd customizedDB &&  $parahash{'programs'}{c} $params_c));
...
system(qq(cd $output_dir/database &&  $parahash{'programs'}{d} $parahash{'params'}{d}));
...
if (!defined($parahash{bypass_database_search}) or $parahash{bypass_database_search}==0)
{
	...
	if (inputRawFiles($ARGV[0]))
	{
		...
		#system(qq(cd $output_dir && $parahash{'programs'}{s} $parahash{'params'}{s} @ARGV));
		system(qq(cd $output_dir &&  $parahash{'programs'}{s} $parahash{'params'}{s} @ARGV));
	} else {
...
if (!defined($parahash{bypass_quality_filter}) or $parahash{bypass_quality_filter}==0)
{
	...
	system(qq(cd $output_dir &&  $parahash{'programs'}{qc} jump_qc.params));
...
if (1)
{
	...
	#system(qq( $parahash{'programs'}{summary} $output_dir/sum_accepted_PSMs $mut_ids $jun_ids $rdt_ids $output_dir/JUMPg_results));

	my $cmd= "$parahash{'programs'}{summary} $output_dir/sum_accepted_PSMs $mut_ids $jun_ids $rdt_ids $output_dir/JUMPg_results";
	#print "$cmd\n";
	system(qq($cmd));
}
...
if (!defined($parahash{bypass_peptide_annotation}) or $parahash{bypass_peptide_annotation}==0)
{
	...
	if (defined($parahash{$mode}) and $parahash{$mode} ne '0')
	{
		...
		system(qq(cd $output_dir/JUMPg_results &&  $parahash{'programs'}{pa} $params_pa));
	}
	$mode='junction';
	if (defined($parahash{$mode}) and $parahash{$mode} ne '0')
	{
...
sub set_params_files
{
	...
	if ($$parahash{search_engine} eq 'JUMP')
	{
		...
		#$$parahash{'params'}{s}="jump_sj_$$parahash{MS_data_types}_human.params";
		#$$parahash{'params'}{f}="jump_fj_$$parahash{MS_data_types}_human.params";
	}
	elsif ($parahash{search_engine} eq 'SEQUEST')
	{
		...
		#$$parahash{'params'}{s}="jump_ss_$$parahash{MS_data_types}_human.params";
		#$$parahash{'params'}{f}="jump_fs_$$parahash{MS_data_types}_human.params";
	}
